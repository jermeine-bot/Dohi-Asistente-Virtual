...
import { getLatestWeight } from '../../../src/service/weightService';
import { WeightRecord } from '../../../src/types/Weight';

export default function HealthHubScreen() {
  const [latestWeight, setLatestWeight] =
    useState<WeightRecord | null>(null);

  const [loadingWeight, setLoadingWeight] =
    useState(true);
  ...
  const loadLatestWeight = async () => {
    try {
      ...
      const record = await getLatestWeight();

      setLatestWeight(record);
    } catch (error) {
      console.error(
        'Error cargando último registro de peso:',
        error
      );
  ...
  /*
   * useFocusEffect permite actualizar los datos
   * cada vez que el usuario vuelve a esta pantalla.
   *
   * Por ejemplo:
   *
   * Health → Peso → Guardar registro → volver a Health
   *
   * El card mostrará inmediatamente el nuevo peso/IMC.
   */
  useFocusEffect(
    useCallback(() => {
      loadLatestWeight();
      ...
      >
        ...
        <View style={styles.gridContainer}>
          ...
          <TouchableOpacity
            activeOpacity={0.88}
            onPress={() =>
              router.push('/(app)/health-campaigns')
            }
            style={[
              styles.moduleCard,
              {
          ...
          <TouchableOpacity
            activeOpacity={0.88}
            onPress={() =>
              router.push('/(app)/epidemiological-alerts')
            }
            style={[
              styles.moduleCard,
              {
          ...
          <TouchableOpacity
            activeOpacity={0.88}
            onPress={() =>
              router.push('/(app)/health-centers')
            }
            style={[
              styles.moduleCard,
              {
